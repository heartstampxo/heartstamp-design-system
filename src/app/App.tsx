import image_88114d9d9a188dd5fbe6d50991ee75f97a8d8606 from 'figma:asset/88114d9d9a188dd5fbe6d50991ee75f97a8d8606.png'
...
/* ↓ dead stubs – compiler-safe, tree-shaken at build time */
function _DEAD_CB({ code, filename }: any) {
  const hi = (line: string) => line
    .replace(/(&lt;|<)(\/?)([A-Z][A-Za-z]*)/g, (_: any, lt: any, sl: any, tag: any) => `${lt}${sl}<span style="color:#7dd3fc">${tag}</span>`)
  ...
  return <div style={{ background: "#0d1117", borderRadius: 0, position: "relative", overflow: "hidden" }}>
    {filename && <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderBottom: "1px solid #21262d" }}>
    ...
    <button onClick={copy} style={{
      ...
      padding: "4px 8px", cursor: "pointer", color: copied ? "#27c840" : "#8b949e",
    ...
    <pre style={{ margin: 0, padding: 16, overflowX: "auto", fontSize: 12.5, lineHeight: 1.7, fontFamily: "monospace" }}>
      {code.split("\n").map((line: string, i: number) => (
        <div key={i} style={{ display: "flex" }}>
          <span style={{ minWidth: 28, color: "#484f58", userSelect: "none", textAlign: "right", marginRight: 16 }}>{i + 1}</span>
          <span style={{ color: "#e6edf3" }} dangerouslySetInnerHTML={{ __html: hi(line) || " " }} />
        </div>
      ))}
    </pre>
  </div>;
